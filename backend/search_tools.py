...
class CourseSearchTool(Tool):
    ...
    def _format_results(self, results: SearchResults) -> str:
        ...
        formatted = []
        sources = []  # Track sources for the UI (as dicts with name and link)
        ...
        for doc, meta in zip(results.documents, results.metadata):
            ...
            # Track source for the UI (both display name and link)
            source_name = course_title
            if lesson_num is not None:
                source_name += f" - Lesson {lesson_num}"
            sources.append({"name": source_name, "link": lesson_link})

            formatted.append(f"{header}\n{doc}")

        # Store sources for retrieval (includes links for frontend)
...
class CourseOutlineTool(Tool):
    ...
    def get_tool_definition(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "description": "Get the complete outline of a course including course title, course link, and all lessons",
            "input_schema": {
                "type": "object",
                "properties": {
                    "course_title": {
    ...
    def execute(self, course_title: str) -> str:
        ...
        try:
            ...
            # Add course link source if available
            if course_link:
                sources.append({"name": f"Course: {title} - link", "link": course_link})

            if lessons_json:
                lessons = json.loads(lessons_json)
                for lesson in lessons:
                    lesson_num = lesson.get("lesson_number")
                    lesson_name = lesson.get("lesson_title", "Untitled")
                    lesson_link = lesson.get("lesson_link")
                    outline_parts.append(f"Lesson {lesson_num}: {lesson_name}")

                    # Add lesson link source if available
                    if lesson_link:
                        sources.append(
                            {
                                "name": f"Lesson {lesson_num}: {lesson_name} - link",
                                "link": lesson_link,
                            }
                        )
            else:
            ...
            return f"Error retrieving outline: {str(e)}"
